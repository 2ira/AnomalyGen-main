...
import logging
import argparse
from models.prompts.merge_node_info import get_merge_nodes_by_llm_without_cot
...
class StackDFSMerger:
    ...
    def _merge_parent(self, node):
        ...
        for child in self.simple_call_graph[node]:
            ...
            prompts = list(get_merge_nodes_by_llm_without_cot(parent_info,child_info))
...
def main():
    ...
    single_log_info =os.path.join(output_dir,"merge_single_info_without_cot.json")
    with open(single_log_info, "w", encoding="utf-8") as f:
        json.dump(single_log_info_json, f, indent=2, ensure_ascii=False)

    # single log seq
    single_log_seq = os.path.join(output_dir,"merge_single_log_without_cot.json")
    with open(single_log_seq, "w", encoding="utf-8") as f:
        json.dump(single_log_seq_json, f, indent=2, ensure_ascii=False)
    print(merged_logs)
    print(f"merge all")
