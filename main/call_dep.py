...
def validate_pruned_graph(pruned_edges, relevant_nodes):
    """verify the outcome"""
    for caller, callees in pruned_edges:
        assert caller in relevant_nodes, f"illegel parent node: {caller}"
        for callee, _ in callees:
            assert callee in relevant_nodes, f"illegel child node: {callee}->{callee}"
...
def main():
    parser = argparse.ArgumentParser(description = "Finish pruning")
    parser.add_argument('--output_dir', type=str, required=False,default="output",help="output dir")
    parser.add_argument('--depth', type=int, required=False,default=3,help="analyze depth")
    parser.add_argument('--entry_function', type=str, required=True,help="set entry function")
